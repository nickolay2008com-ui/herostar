...
const safe = async (name, fn, severity = 'blocker') => {
  try {
...
async function writeReport() {
  ...
  const report = {
    ...
    summary: { total: checks.length, passed: checks.filter(c => c.ok).length, blockers: blockers.length, warnings: warnings.length },
    checks,
  };
...
try {
  ...
  for (const asset of ['/styles.css','/app.js','/analytics.js','/treasure-experience.js','/place-autocomplete.js','/deep-dive-ui.js','/typography.css','/placidus.css']) {
    await safe(`Ресурс ${asset}`, async () => {
      const response = await fetch(new URL(asset, BASE));
      return { ok: response.ok, details: `${response.status}` };
    });
  }
  ...
  const demo = await json('/api/charts', {
    method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Visitor-Id': visitorId }, body: JSON.stringify({ demo: true }),
  });
  ...
  const event = await json('/api/events', {
    method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Visitor-Id': visitorId },
  ...
  for (const viewport of [
    ...
    try {
      ...
      add(`${viewport.name}: нет горизонтального переполнения`, geometry.overflow <= 1, JSON.stringify(geometry));
      ...
      if (viewport.name === 'mobile') {
        await safe('mobile: список городов появляется', async () => {
          ...
          return { ok: count > 0, details: `options=${count}` };
        });
        await safe('mobile: выбранный город подтверждён', async () => {
          await page.locator('.place-option').first().click();
          await page.locator('#placeStatus.success').waitFor({ state: 'visible', timeout: 5000 });
          return { ok: true, details: await page.locator('#placeStatus').textContent() };
        });
        await safe('mobile: карта открылась', async () => {
          await page.locator('#demoButton').click();
          await page.locator('#map:not(.hidden)').waitFor({ state: 'visible', timeout: 45000 });
          return { ok: await page.locator('#map').isVisible() };
        });
        ...
        await safe('mobile: глубокое окно открывается', async () => {
          const firstCard = page.locator('.treasure-card').first();
          const className = await firstCard.getAttribute('class') || '';
          if (!className.includes('open')) await firstCard.locator('.card-trigger').click();
        ...
        await safe('mobile: paywall ведёт к Telegram', async () => {
          const locked = page.locator('.treasure-card.locked-card').first();
          await locked.scrollIntoViewIfNeeded();
          const className = await locked.getAttribute('class') || '';
          if (!className.includes('open')) await locked.locator('.card-trigger').click();
          await locked.locator('[data-open-pay]').click();
          await page.locator('#authModal:not(.hidden)').waitFor({ state: 'visible', timeout: 10000 });
          return { ok: true };
        });
        add('mobile: Telegram widget создаётся', await page.locator('#telegramSlot script[data-telegram-login]').count() === 1, `count=${await page.locator('#telegramSlot script[data-telegram-login]').count()}`);
        await page.screenshot({ path: `${outDir}/mobile-map.png`, fullPage: true }).catch(() => {});
      }
    } catch (error) {
      add(`${viewport.name}: браузерный сценарий завершён`, false, error?.message || String(error));
    } finally {
      await page.close();
    }
  }
  ...
  const blockerCount = await writeReport();
