...
async function safeTrack(record) {
  try { await trackEvent(record); } catch (error) { console.error('Payment analytics event was not saved:', error); }
}
...
function providerError(response, payload) {
  ...
  const error = publicError('ЮKassa не смогла создать платёж. Повторите ещё раз или напишите @ainicki.', 502, 'PAYMENT_PROVIDER_ERROR');
  error.providerErrorId = providerId;
  return error;
}
...
async function yookassaRequest(path, options = {}) {
  ...
  try {
    response = await fetch(\`https://api.yookassa.ru/v3\${path}\`, {
      ...options,
      headers: { Authorization: \`Basic \${Buffer.from(\`\${shopId}:\${secretKey}\`).toString('base64')}\`, 'Content-Type': 'application/json', ...(options.headers || {}) },
...
export async function createPayment({ user, chartId, visitorId = null, receiptContact }) {
  ...
  const body = {
    ...
    description: 'HeroStar — полный доступ к интерактивной карте',
  ...
  await safeTrack({ eventType: 'payment_created', visitorId, userId: user.telegram_id, chartId: chartId || null, metadata: { paymentId: payment.id, amount: Number(amount), status: payment.status } });
  return payment;
}
...
export async function processWebhook(notification) {
  ...
  if (payment.status === 'succeeded' && payment.paid) {
    ...
    await safeTrack({ eventType: 'payment_succeeded', userId: userId || null, chartId: chartId || null, metadata: { paymentId: payment.id, amount: Number(payment.amount?.value || 0), currency: payment.amount?.currency || 'RUB' } });
  }
  return payment;
}
`);
...
await fs.rm('.github/scripts/apply-payment-repair.mjs');
await fs.rm('.github/workflows/apply-payment-repair.yml');
