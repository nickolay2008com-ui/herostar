...
const PAYMENT_KEY = 'starCloneLivePayment';
...
const cusp = (chart, house) => (chart?.houses?.cusps || []).find((item) => Number(item.house) === Number(house)) || null;
...
function actionPlanetKey(category) {
  if (['relationship','money','shared'].includes(category.id)) return 'venus';
...
function buildDemo(chart, category) {
  ...
  return {
    house:category.house, sign, cusp:activeCusp, ruler, support, decision, why, contrast,
    advantage, firstMove:move,
...
async function createChart(form) {
  ...
  if (data.get('personalDataConsent') !== 'on') throw new Error('Подтвердите согласие на обработку данных рождения.');

  const name = String(data.get('name') || '').trim();
  const result = await json('/api/charts', { method:'POST', body:JSON.stringify({
    name, date:data.get('date'), time:data.get('time'), place,
    personalDataConsent:true, visitorId:visitorId(), product:'clone', experience:'live',
  }) });
  ...
  persist();
  await track('card_opened', 'clone_live_created', { category:state.category.id, house:state.category.house });
  return result;
}
...
function displayUserMessage(content = '') {
  const marker = 'Новая реплика человека:';
  if (!content.includes(marker)) return content;
  return content.split(marker)[1].split('\n\nНе начинай разбор заново.')[0].trim();
}

async function loadHistory() {
  if (!state.user || !state.chartId) return;
  try {
    const data = await json(`/api/charts/${encodeURIComponent(state.chartId)}/messages`);
    const messages = (data.messages || []).filter((item) => item.metadata?.product === 'clone').slice(-30);
    $('#messages').innerHTML = '';
    if (!messages.length) {
      addMessage('clone', 'Карта, исходная ситуация и первый разбор сохранены. Продолжайте с новым обстоятельством или уточнением.');
      return;
    }
    for (const item of messages) {
      addMessage(item.role === 'assistant' ? 'clone' : 'user', item.role === 'user' ? displayUserMessage(item.content) : item.content);
    }
  } catch {
    if (!$('#messages').children.length) addMessage('clone', 'История сохранена. Продолжайте с нового обстоятельства.');
  }
}
...
async function claimChart() {
  if (state.chartId && state.user) await json(`/api/charts/${encodeURIComponent(state.chartId)}/claim`, { method:'POST', body:'{}' });
}
...
function startAuthPoll() {
  ...
  state.authPoll = setInterval(async () => {
    ...
    try {
      const config = await json(`/api/config?chartId=${encodeURIComponent(state.chartId || '')}`);
      if (!config.user) return;
      state.config = config;
      state.user = config.user;
      await claimChart();
      stopAuthPoll();
      await track('filter_changed', 'clone_live_login_succeeded');
      goal('clone_live_telegram_saved');
      await revealDialogue();
    } catch {}
  }, 1200);
...
async function mountTelegram() {
  ...
  if (!state.config?.telegramConfigured) {
    slot.textContent = 'Вход временно недоступен. Проверьте подключение Telegram-бота.';
    return;
  }
  await track('auth_opened', 'clone_live_auth_opened');
  ...
  slot.innerHTML = '<span>После входа экран продолжит работу автоматически.</span>';
  slot.prepend(script);
  startAuthPoll();
}
...
async function ask(question) {
  ...
  try {
    const result = await json('/api/consult', { method:'POST', body:JSON.stringify({
      chartId:state.chartId, question:prepared.text, product:'clone', experience:'live',
    }) });
    pending.querySelector('p').textContent = result.answer;
    if (prepared.carriesContext) {
      state.contextSynced = true;
      persist();
    }
    await track('consultant_opened', 'clone_live_dialogue_answered', { answerLength:String(result.answer || '').length });
  } catch (error) {
    pending.remove();
    if (error.code === 'CLONE_FREE_LIMIT') openPaywall();
    else $('#dialogueError').textContent = error.message;
  } finally {
    setAsking(false);
  }
}
...
function renderOfferButtons() {
  ...
  if (state.user?.cloneAccessActive) {
    $('#accessBadge').textContent = state.user.clonePlan === 'alignment' ? 'Доступ на 30 дней активен' : 'Платный доступ активен';
  } else if (state.user) {
    $('#accessBadge').textContent = 'Первый день открыт бесплатно';
  }
}
...
function receiptContact() {
  ...
  const email = /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(raw);
  const digits = raw.replace(/\D/g, '');
  if (!email && (digits.length < 10 || digits.length > 15)) {
...
async function startPayment(offerCode) {
  ...
  try {
    const result = await json('/api/payments/create', { method:'POST', body:JSON.stringify({
      chartId:state.chartId, receiptContact:contact, product:'clone_live', offerCode,
    }) });
    if (!result.confirmationUrl || !result.paymentRef) throw new Error('ЮKassa не вернула данные оплаты.');
    localStorage.setItem(PAYMENT_KEY, JSON.stringify({
      paymentId:result.paymentId || null, paymentRef:result.paymentRef,
      chartId:state.chartId, offerCode, createdAt:new Date().toISOString(),
    }));
...
function pendingPayment() {
  try { return JSON.parse(localStorage.getItem(PAYMENT_KEY) || 'null'); }
  catch { return null; }
}
...
async function verifyPaymentReturn() {
  ...
  const ref = params.get('payment_ref') || pending?.paymentRef;
  if (!ref || !state.user) {
    toast('Платёж вернулся без подтверждаемой ссылки. История клона сохранена.');
    return;
  }

  toast('Проверяем подтверждение оплаты…');
  for (let attempt = 0; attempt < 7; attempt += 1) {
    try {
      const query = new URLSearchParams({ ref });
      if (pending?.paymentId) query.set('paymentId', pending.paymentId);
      const status = await json(`/api/payments/status?${query}`);
      if (status.paid) {
        localStorage.removeItem(PAYMENT_KEY);
        state.config = await json(`/api/config?chartId=${encodeURIComponent(state.chartId || status.chartId || '')}`);
        state.user = state.config.user;
        renderOfferButtons();
        await revealDialogue();
        history.replaceState({}, '', '/clone/live/');
        goal('clone_live_payment_success');
        toast('Оплата подтверждена. Диалог продолжен с сохранённой историей.');
        return;
      }
      if (['canceled', 'cancelled'].includes(status.status)) {
        localStorage.removeItem(PAYMENT_KEY);
        toast('Платёж отменён. История и карта остаются сохранёнными.');
        return;
      }
    } catch (error) {
      if (error.code === 'PAYMENT_FORBIDDEN') {
        toast('Для проверки оплаты войдите тем же Telegram-аккаунтом.');
        return;
      }
    }
    await sleep(1500 + attempt * 500);
  }
  toast('ЮKassa ещё обрабатывает платёж. Обновите страницу через минуту — история не потеряется.');
}
...
async function restore() {
  ...
  Object.assign(state, {
    situation:saved.situation, category:saved.category || classifySituation(saved.situation),
    chartId:saved.chartId, token:saved.token, cloneName:saved.cloneName || 'Ваш Звёздный клон',
    demo:saved.demo, contextSynced:Boolean(saved.contextSynced),
  });
  try {
    const [chartData, config] = await Promise.all([
      json(`/api/charts/${encodeURIComponent(state.chartId)}`),
      json(`/api/config?chartId=${encodeURIComponent(state.chartId)}`),
    ]);
...
function bindSituation() {
  ...
  $('#situationForm').addEventListener('submit', (event) => {
    ...
    if (situation.length < 20) {
      toast('Опишите ситуацию немного подробнее.');
      return;
    }
  ...
  $('#placeQuery').addEventListener('input', () => {
    ...
    timer = setTimeout(async () => {
      try {
        ...
        $('#placeResults').innerHTML = (data.items || []).slice(0, 7)
          .map((item, index) => `<button type="button" data-index="${index}">${escapeHtml(item.label || item.name)}</button>`).join('');
...
function bindBirth() {
  $('#birthForm').addEventListener('submit', async (event) => {
    ...
    const titles = [
      'Определяем жизненную сферу ситуации', 'Читаем знак и градус куспида',
      'Находим управителя дома', 'Ищем преимущество в гармоничных аспектах',
    ...
    const interval = setInterval(() => {
      ...
      $$('.building-steps span').forEach((node, i) => node.classList.toggle('active', i <= Math.min(index, 3)));
    }, 720);
    try {
