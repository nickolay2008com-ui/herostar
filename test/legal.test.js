...
test('production-оплата невозможна без опубликованных реквизитов и обязательных настроек', () => {
  const server = read('server.js');
  const app = read('public/app.js');
  const readiness = read('src/production-readiness.js');
  assert.match(server, /requirePaymentReadiness/);
  assert.match(readiness, /getLegalConfig\(env\)\.configured/);
  assert.match(readiness, /PAYMENTS_NOT_READY/);
