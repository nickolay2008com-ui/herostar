...
test('системный промпт клона не дублирует тарифную инструкцию вопроса', () => {
  ...
  assert.ok(!deep.includes('Это первый содержательный ответ Звёздного клона'));
  assert.ok(!deep.includes('различающий вопрос без преждевременного большого разбора'));
  assert.ok(!deep.includes('механике активного профиля'));
});
