...
test('сервер атомарно пропускает только три анонимных ответа до Telegram', async () => {
  const chartId = `test-${crypto.randomUUID()}`;
  const attempts = await Promise.all(
    Array.from({ length: 4 }, () => reserveCloneQuestion({ chartId, userId: null, limit: 3 })),
  );
...
test('неудавшийся ответ освобождает бесплатный вопрос', async () => {
  const chartId = `test-${crypto.randomUUID()}`;
  const reservation = await reserveCloneQuestion({ chartId, userId: null, limit: 3 });
...
test('анонимный лимит привязан к карте, а после Telegram бесплатный диалог продолжается без продуктовой квоты', async () => {
