...
test('политика диалога ведёт к ресурсам, а не к лечению человека', () => {
  ...
  assert.match(prompt, /единство/i);
  assert.match(prompt, /уникальность/i);
});

test('глубокий ответ остаётся естественным, а не превращается в отчёт', () => {
  const prompt = consultationSystemPrompt('deep');
  assert.match(prompt, /не используй обязательные заголовки/i);
  assert.match(prompt, /не используй видимые рубрики/i);
