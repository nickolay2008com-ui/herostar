...
const gearsPath = new URL('../public/clone-ui-gears.js', import.meta.url);
const gears = readFileSync(gearsPath, 'utf8');
...
test('clone live frontend and server helpers have valid JavaScript', () => {
  for (const path of [jsPath, gearsPath, quotaPath, paymentsPath, commercePath]) {
...
test('mobile Telegram return is handed back to the live experience', () => {
  assert.match(gears, /LIVE_STATE_KEY = 'starCloneLive'/);
  assert.match(gears, /returnedChartId === String\(liveState\.chartId\)/);
  assert.ok(gears.includes('/clone/live/?auth=ok&chart='));
  assert.match(gears, /if \(authReturned && window\.opener/);
});
...
test('approved tariffs are 7 days for 490 and 30 days for 990 without autorenew', () => {
  ...
  assert.match(commerce, /amount: money\(env\.CLONE_WEEK_PRICE, 490\)/);
  assert.match(commerce, /durationHours: 7 \* 24/);
  assert.match(commerce, /title: '30 дней \+ полная карта HeroStar'/);
  assert.match(commerce, /amount: money\(env\.CLONE_MONTH_PRICE, 990\)/);
  assert.match(commerce, /INTERVAL '7 days'/);
  assert.match(payments, /Диалог со Звёздным клоном на 7 дней/);
  assert.doesNotMatch(commerce, /autoRenew/);
});

test('only the 30-day plan permanently unlocks the full map', () => {
  const weeklyBranch = commerce.split("offerCode === OFFER_CODES.CLONE_DAY")[1].split("offerCode === OFFER_CODES.CLONE_ALIGNMENT")[0];
  assert.doesNotMatch(weeklyBranch, /full_map_unlocked = true/);
  assert.match(commerce, /offerCode === OFFER_CODES.CLONE_ALIGNMENT[\s\S]*full_map_unlocked = true/);
});
