...
def replace_all(text, old, new, expected, label):
    count = text.count(old)
    if count != expected:
        raise SystemExit(f'{label}: expected {expected} fragments, found {count}')
...
bootstrap = replace_all(
    ...
    'startTelegramUpdateRuntime',
    2,
    'bootstrap runtime naming',
)
write(bootstrap_path, bootstrap)

contract_path = 'test/telegram-link-auth.test.js'
contract = read(contract_path)
contract = replace_all(
    contract,
    'startTelegramLinkUpdatePolling',
    'startTelegramUpdateRuntime',
    1,
    'contract runtime naming',
)
...
print('Unified Telegram runtime finalized successfully.')
