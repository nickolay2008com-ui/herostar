...
Карточки должны идти строго: sun, moon, mercury, venus, mars, jupiter, saturn, uranus, neptune, pluto, northNode. Категории сохраняй по смыслу.`,
    ...
    const portrait = JSON.parse(response.output_text);
    const expectedOrder = ['sun', 'moon', 'mercury', 'venus', 'mars', 'jupiter', 'saturn', 'uranus', 'neptune', 'pluto', 'northNode'];
    if (!portrait.cards?.every((card, index) => card.id === expectedOrder[index])) {
      throw new Error('OpenAI returned cards in an unsafe order.');
    }
    return { portrait, source: 'openai' };
  } catch (error) {
    console.error('OpenAI portrait fallback:', error?.message || error);
