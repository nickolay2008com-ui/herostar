...
const NATURAL_LANGUAGE_FOOTER = 'Отвечай максимально естественно, живо и понятно — простыми человеческими словами, без канцелярита и шаблонных фраз.';

function withNaturalLanguageFooter(prompt) {
  return `${String(prompt || '').trim()}\n\n${NATURAL_LANGUAGE_FOOTER}`;
}
...
export function consultationSystemPrompt(mode, product = 'herostar', premium = false, hasExternalContext = false) {
  ...
  const cloneGroundingPolicy = isClone
    ? `В пользовательском контексте есть selectedFactors — это факторы карты, заранее выбранные продуктом как доказательная основа именно текущего ответа. Основной вывод и объяснение «почему именно так» должны быть совместимы с selectedFactors. Сначала синтезируй их в одну живую механику, а не перечисляй по одному. Не копируй служебное поле role дословно: переводи его в естественный человеческий смысл. Полная chart передана как фон для понимания положений и связей, но не подменяй selectedFactors случайными другими факторами при обосновании ответа. Если выбранных факторов недостаточно для честного конкретного вывода, не придумывай связь: кратко обозначь ограничение и задай один уточняющий вопрос, который действительно изменит выбор факторов.`
    : '';
...
Не вставляй упражнение в каждый ответ насильно. Не заканчивай каждый ответ вопросом: вопрос нужен только тогда, когда он меняет дальнейший маршрут. Завершай естественно — ясным выводом, мягким выбором направления или одним точным вопросом.
