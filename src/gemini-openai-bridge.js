...
async function callGemini(originalFetch, consultation, payload, model, { timeoutMs, fallback = false } = {}) {
...
function remainingDeadlineMs(startedAt) {
  return Math.max(0, LIVE_AI_DEADLINE_MS - (Date.now() - startedAt));
}
